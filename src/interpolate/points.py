...
from interpolate.general import interpolation_weights_1d
from tools.math import wrapped_difference
from tools.grid import enclosing_points_1d
...
def interpolate_points_nd(
        ...
        get_data:Callable[ [List[np.ndarray]],np.ndarray],
        period_data,
        discont,
        output_shape = None,
    ...
    indices_1d = np.empty((number_interp_coor, 2, number_points), dtype='int64')
    weights_1d = np.empty((number_interp_coor, 2, number_points), dtype='float64')
    ...
    for intp_indices_nd, intp_weight_nd in _next_point(
        ...
        if period_data is not None:
            to_rad = np.pi * 2 / period_data
            val = np.exp(1j * get_data(intp_indices_nd) * to_rad)
        else:
            try:
    ...
    interp_val = np.where(
        weights_sum > 0.5, interp_val / weights_sum, np.nan)

    if period_data is not None:
        to_data_units =  period_data / np.pi / 2
        interp_val = wrapped_difference(
            delta=np.angle(interp_val) * to_data_units,
            period=period_data,
            discont=discont)
