...
def create_fourier_amplitudes(
    ...
    phases = np.random.default_rng(seed=seed).uniform(0, 2 * np.pi, spectrum.spectral_shape())
    amplitudes = np.sqrt(area * spectrum.variance_density / 2) * np.exp(1j * phases) * factor
