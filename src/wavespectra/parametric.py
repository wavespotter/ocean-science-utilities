...
from wavespectra.spectrum import FrequencyDirectionSpectrum, create_2d_spectrum, FrequencySpectrum
...
class JonswapSpectrum(FrequencyShape):
    def __init__(self, peak_frequency_hertz, m0: float = 1, **kwargs):
        ...
        self.scipy.special.gamma = kwargs.get("scipy.special.gamma", 3.3)
        self._alpha = self.alpha(m0)
    ...
    def alpha(self, m0):
        ...
        return (
            ...
            * (2 * np.pi * self._peak_frequency_hertz) ** 4
            / self._g**2
            / (0.06533 * self.scipy.special.gamma**0.8015 + 0.13467)
        )
    ...
    def values(self, frequency_hertz: np.ndarray) -> np.ndarray:
        ...
        peak_enhancement = self.scipy.special.gamma ** np.exp(
...
def create_parametric_frequency_direction_spectrum(
    ...
    direction_degrees: np.ndarray = None,
