...
from interpolate.dataset import interpolate_dataset_grid, interpolate_dataset_along_axis
from tools.grid import midpoint_rule_step
from tools.math import wrapped_difference
from tools.time import to_datetime64
from wavetheory.lineardispersion import (
    inverse_intrinsic_dispersion_relation,
    intrinsic_group_velocity,
)
from wavespectra.estimators.estimate import estimate_directional_distribution, Estimators
from wavespectra._tools import (
...
class WaveSpectrum(DatasetWrapper):
    ...
    def where(self: _T, condition: xarray.DataArray) -> _T:
        return self._apply_filter(condition)

    def _apply_filter(self: _T, boolean_mask: xarray.DataArray) -> _T:
        dataset = xarray.Dataset()
        for var in self.dataset:
    ...
    def hm0(self, fmin=0, fmax=np.inf) -> xarray.DataArray:
        ...
        """
        return 4 * np.sqrt(self.m0(fmin, fmax))
    ...
    def tm02(self, fmin=0, fmax=np.inf) -> xarray.DataArray:
        ...
        """
        return np.sqrt(self.m0(fmin, fmax) / self.m2(fmin, fmax))

    def peak_index(self, fmin=0, fmax=np.inf) -> xarray.DataArray:
        """
    ...
    def _mean_direction(a1: _T, b1: _T) -> _T:
        return np.arctan2(b1, a1) * 180 / np.pi

    @staticmethod
    def _spread(a1: _T, b1: _T) -> _T:
        return np.sqrt(2 - 2 * np.sqrt(a1**2 + b1**2)) * 180 / np.pi
    ...
    def wavenumber(self) -> xarray.DataArray:
        ...
        # For numba (used in the dispersion relation) we need raw np arrays of the correct dimension
        depth = self.depth.expand_dims(dim=NAME_F, axis=-1).values
    ...
    def extrapolate_tail(
        ...
        variance_density = xarray.concat(
            (e, e.isel(frequency=-1) * xarray.zeros_like(tail_frequency)), dim="frequency"
        )
        ...
        a1 = xarray.concat((a1, tail_a1 * xarray.ones_like(tail_frequency)), dim="frequency")
        b1 = xarray.concat((b1, tail_b1 * xarray.ones_like(tail_frequency)), dim="frequency")
        a2 = xarray.concat((a2, tail_a2 * xarray.ones_like(tail_frequency)), dim="frequency")
        b2 = xarray.concat((b2, tail_b2 * xarray.ones_like(tail_frequency)), dim="frequency")
    ...
    def bandpass(self: _T, fmin=0, fmax=np.inf) -> _T:
...
class FrequencyDirectionSpectrum(WaveSpectrum):
    ...
    def _directionally_integrate(self, data_array: xarray.DataArray) -> xarray.DataArray:
    ...
    def differentiate(self, coordinate=None, **kwargs) -> "FrequencyDirectionSpectrum":
        ...
        return FrequencyDirectionSpectrum(xarray.Dataset(data_vars=data, coords=self.coords()))
...
def create_1d_spectrum(
    ...
    if a1 is None:
        a1 = np.nan + np.xarray.ones_like(variance_density)
    if b1 is None:
        b1 = np.nan + np.xarray.ones_like(variance_density)
    if a2 is None:
        a2 = np.nan + np.xarray.ones_like(variance_density)
    if b2 is None:
        b2 = np.nan + np.xarray.ones_like(variance_density)
