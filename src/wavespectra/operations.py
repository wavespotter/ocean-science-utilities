    ...
    if NAME_D in dims:
        difference = xarray.DataArray(
            data=wrapped_difference(
                np.diff(dataset.direction.values, append=dataset.direction[0]), period=360
            ),
...
def numba_integrate_spectral_data(data: np.typing.NDArray, grid):
...
@njit(cache=True, fastmath=True)
def numba_directionally_integrate_spectral_data(data: np.typing.NDArray, grid):
