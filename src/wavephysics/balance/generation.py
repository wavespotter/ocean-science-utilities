...
from wavephysics.balance._numba_settings import numba_default, numba_nocache_parallel
from wavephysics.balance.source_term import SourceTerm
from wavephysics.balance.stress import _roughness_estimate, _wave_supported_stress, _tail_supported_stress
from wavespectra.operations import numba_integrate_spectral_data
...
class WindGeneration(SourceTerm):
    ...
    def rate(
        ...
        roughness_length: xarray.DataArray = None,
        ...
        return xarray.DataArray(data=wind_input, dims=spectrum.dims, coords=spectrum.coords())
    ...
    def roughness(
        ...
        direction: xarray.DataArray,
        spectrum: FrequencyDirectionSpectrum,
        roughness_length_guess: xarray.DataArray = None,
        wind_speed_input_type: TWindInputType = "u10",
    ) -> xarray.DataArray:
    ...
    def drag(
        ...
        if wind_speed_input_type == "u10":
            ...
            u10 = u_star * self.parameters["vonkarman_constant"] * np.log(10 / roughness)
...
def _wind_generation(
    ...
    generation = np.empty((number_of_points, number_of_frequencies, number_of_directions))
...
def _numba_parameters(**kwargs):
    _dict = numba.typed.Dict.empty(key_type=numba.types.unicode_type, value_type=numba.types.float64)
    for key in kwargs:
