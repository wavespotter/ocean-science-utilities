...
from wavephysics.balance._numba_settings import numba_nocache_parallel, numba_nocache, numba_default
from wavephysics.balance.balance import SourceTermBalance
from wavephysics.balance.dissipation import _bulk_dissipation_direction_point
from wavephysics.balance.solvers import numba_newton_raphson
from wavephysics.balance.stress import _roughness_estimate_point, _total_stress_point
...
def windspeed_and_direction_from_spectra(
    ...
    if time_derivative_spectrum is None:
        time_derivative_spectrum = np.zeros(spectrum.shape())
    else:
    ...
    if jacobian:
        return xarray.Dataset(data_vars={"u10": u10, "direction": direction, "jacobian": grad})
    else:
