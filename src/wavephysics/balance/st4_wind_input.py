...
from wavephysics.balance._numba_settings import numba_default
from wavephysics.balance.generation import WindGeneration
from wavephysics.balance.wam_tail_stress import tail_stress_parametrization_wam
from wavephysics.fluidproperties import AIR, WATER, GRAVITATIONAL_ACCELERATION
from wavetheory.lineardispersion import inverse_intrinsic_dispersion_relation
...
def _st4_wind_generation_point(
    ...
    cosine_mutual_angle_wind_waves = np.cos(
        (spectral_grid["radian_direction"] - wind_direction_degrees * np.pi / 180 + np.pi)
        % (2 * np.pi)
        - np.pi
    )
