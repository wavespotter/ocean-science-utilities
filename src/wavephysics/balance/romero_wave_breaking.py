...
from wavephysics.fluidproperties import GRAVITATIONAL_ACCELERATION
from wavetheory.lineardispersion import (
    inverse_intrinsic_dispersion_relation,
    intrinsic_group_velocity,
)
from wavespectra.operations import numba_directionally_integrate_spectral_data
...
def romero_dissipation_breaking(
    variance_density: np.typing.NDArray, depth: np.typing.NDArray, spectral_grid, parameters
):
    ...
    for frequency_index in range(number_of_frequencies):
        delta = np.sqrt(saturation[frequency_index]) - np.sqrt(saturation_integrated_threshold)
        if delta > 0.0:
