...
def wavenumber_grid(
    ...
    if log_bounds[1] < log_starting_wavenumber:
        # Numba does not handle multiple returns well. Here I return a len 1 np.array with
...
def saturation_spectrum_parametrization(
    ...
    strength_three_wave_interactions = (
        3 * np.pi / 16 * (np.tanh(2 * (np.sqrt(wavenumbers / three_wave_start) - 1)) + 1)
    )
...
def three_wave_starting_wavenumber(friction_velocity, parameters):
    ...
    if parameters["surface_tension"] == 0.0:
        return np.inf
    return (
        np.sqrt(parameters["gravitational_acceleration"] / parameters["surface_tension"])
        * 1
...
def celerity(wavenumber, gravitational_acceleration, surface_tension):
    return np.sqrt(gravitational_acceleration / wavenumber + surface_tension * wavenumber)
