...
def datetime64_to_timestamp(time):
    return (time - np.datetime64(0, "s")) / np.timedelta64(1, "s")
...
def to_datetime64(time) -> Union[None, np.datetime64, np.typing.NDArray[np.datetime64]]:
    ...
    if isinstance(time, datetime):
        ...
        return np.array([np.datetime64(int(x.timestamp()), "s").astype('<M8[ns]') for x in time])
