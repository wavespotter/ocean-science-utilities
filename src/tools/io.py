...
from tools.time import to_datetime_utc
from wavespectra.spectrum import (
    FrequencySpectrum,
...
def _b64_encode_numpy(val: np.ndarray) -> dict:
    if val.dtype == np.float64:
        val = val.astype(dtype=np.float32)
...
def _b64_decode_numpy(data):
    ...
    return np.reshape(array, data["shape"])


def _b64_encode_dataset(val: xarray.Dataset, name) -> dict:
    net_cdf = val.to_netcdf(engine="scipy")
    data = base64.b64encode(net_cdf)
    return {"__class__": name, "data": {"data": data.decode("utf8")}}


def _b64_encode_dataarray(val: xarray.DataArray, name) -> dict:
...
def _b64_decode_dataset(data) -> xarray.Dataset:
    decoded_bytes = base64.b64decode(bytes(data["data"], encoding="utf-8"))
    with BytesIO(decoded_bytes) as fp:
        dataset = xarray.open_dataset(fp, engine="scipy")
    return dataset


def _b64_decode_dataarray(data) -> xarray.DataArray:
    decoded_bytes = base64.b64decode(bytes(data["data"], encoding="utf-8"))
    with BytesIO(decoded_bytes) as fp:
        dataset = xarray.open_dataset(fp, engine="scipy")
...
class NumpyEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, np.ndarray):
            ...
            return _b64_encode_dataset(obj, "xarray.Dataset")
        elif isinstance(obj, xarray.DataArray):
            return _b64_encode_dataarray(obj, "xarray.DataArray")
            ...
            return {"__class__": "pd.DataFrame", "data": obj.to_json(date_unit="s")}
        else:
...
def object_hook(dictionary: dict):
    if "__class__" in dictionary:
        if dictionary["__class__"] == "pd.DataFrame":
            # This is a mess- needs to be refactored.
            df = pd.read_json(dictionary["data"])
            if "timestamp" in df:
        ...
        elif dictionary["__class__"] == "xarray.Dataset":
            return _b64_decode_dataset(dictionary["data"])
        elif dictionary["__class__"] == "xarray.DataArray":
