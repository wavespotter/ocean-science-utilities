...
@njit(cache=True)
def integrate(
    time: np.typing.NDArray, signal: np.typing.NDArray, order=DEFAULT_ORDER, n=DEFAULT_N, start_value=0.0
) -> np.typing.NDArray:
    """
...
def cumulative_distance(latitudes, longitudes):
    ...
    for ii in range(1, len(latitudes)):
        ...
        # reduced latitude
        reduced_latitude = np.arctan(np.sqrt(1 - eccentricity_squared) * np.tan(mean_latitude))
...
def complex_response(
    normalized_frequency: np.typing.NDArray, order: int, number_of_implicit_points: int = 1
):
    """
...
@njit(cache=True)
def integration_stencil(order: int, number_of_implicit_points: int = 1) -> np.typing.NDArray:
    """
