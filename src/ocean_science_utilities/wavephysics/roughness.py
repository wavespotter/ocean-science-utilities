...
def charnock_roughness_length(
    friction_velocity: xarray.DataArray, **kwargs
) -> xarray.DataArray:
...
def charnock_roughness_length_from_u10(speed, **kwargs) -> xarray.DataArray:
    ...
    visc = kwargs.pop("viscous_constant", 0.0)

    def _func(roughness):
        friction_velocity = (
            air.vonkarman_constant * speed / np.log(elevation / roughness)
        )
        return charnock_roughness_length(
            friction_velocity, charnock_constant=const, viscous_constant=visc
        )
...
def janssen_roughness_length(
    ...
    wind_direction: Optional[xarray.DataArray] = None,
):
...
def drag_coefficient(
    u10: xarray.DataArray, roughness: xarray.DataArray, **kwargs
) -> xarray.DataArray:
    air = kwargs.get("air", AIR)
    elevation = kwargs.get("elevation", 10)
