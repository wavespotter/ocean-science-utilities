...
class ST4WindInput(WindGeneration):
    ...
    def __init__(self, parameters: Optional[ST4WaveGenerationParameters] = None):
...
def _st4_wind_generation_point(
    ...
    """
    Implementation of the st4 wind growth rate term as proposed by Janssen 1991-
    and as described in the ST4 paper.

    :param variance_density: input numpy array of shape (nf,nd), with nf number of
        frequencies, nd number of directions, that contains variance
        densities (unit m**2/Hz/Degree).
    :param wind: Wind input tuple (speed, direction, type) that descripes the wind
        speed, direction and kind of wind input. The latter can be: "u10"
        or "friction_velocity".
    :param depth: Local water depth in meter.
    :param roughness_length: Roughness length in meter.
    :param spectral_grid: numba dictionary containing the spectral grid
        (frequencies/directions)
    :param parameters: numba dictionary containing the parameters of the st4 algorithm.
        See the st4 class for details.
    :param wind_source: optional numpy array that contains the return values.
        This helps to avoid reallocating memory if the routine is called often
        in successive fashion (e.g. during iterative solutions).

    :return: numpy array of shape (nf,nd) containing for each frequency/direction
        pair the wind input in (m**2/Hz/Degree/s)
    """
    ...
    cosine_mutual_angle_wind_waves = np.cos(
        (
            spectral_grid["radian_direction"]
            - wind_direction_degrees * np.pi / 180
            + np.pi
        )
        % (2 * np.pi)
        - np.pi
    ...
    if wind_forcing_type == "u10":
        friction_velocity = (
            wind_forcing
            * vonkarman_constant
            / np.log(parameters["elevation"] / roughness_length)
        )
    ...
    for frequency_index in range(number_of_frequencies):
        ...
        for direction_index in range(number_of_directions):
            ...
            if cosine_mutual_angle_wind_waves[direction_index] > 0:
                ...
                # The logarithm of the approximate expression for the dimensionless
                # critical height (Ardhuin 2010, eq 20).
                log_dimensionless_critical_height = np.log(
                    wavenumber[frequency_index] * roughness_length
                ) + vonkarman_constant / (W + cosine_wave_age_tuning[direction_index])

                # if the dimensionless critical height > 1 we assume there is no more
                # energy transfer (Janssen 1991), his equation 18.
                if log_dimensionless_critical_height > 0:
                    log_dimensionless_critical_height = 0

                growth_rate = (
                    constant_factor
                    * np.exp(log_dimensionless_critical_height)
